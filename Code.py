...
if st.button("🔍 Find Similar"):
    ...
    st.dataframe(results_df[['track_name', 'artists']].reset_index(drop=True, index=False))


import plotly.graph_objects as go

st.subheader("📈 Audio Feature Comparison (Radar Chart)")

# Pobierz wektor cech utworu wejściowego
query_features = df_norm.loc[selected_index, features_for_similarity].values

# Radar plot
fig = go.Figure()

# Dodaj wybrany utwór (bazowy)
fig.add_trace(go.Scatterpolar(
    r=query_features,
    theta=features_for_similarity,
    fill='toself',
    name='Selected Track',
    line=dict(color='black', width=2)
))

# Dodaj każdy z podobnych utworów
for i, row in results_df.iterrows():
    ...
    if len(match_idx) > 0:
        r_values = df_norm.loc[match_idx[0], features_for_similarity].values
        fig.add_trace(go.Scatterpolar(
            r=r_values,
            theta=features_for_similarity,
            fill='toself',
            name=f"{row['track_name']} – {row['artists']}"
        ))
